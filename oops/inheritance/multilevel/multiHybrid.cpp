...
class Lerbra : protected Dog
{

public:
    int getDogProtectedWithLerbra()
    {
        return length;
    }
    int getDogPublicWithLerbra()
    {
        return height;
    }
    int getAnimal_DogProtectedWithLerbra()
    {
        return getAnimalPrivate();
    }
};
...
int main()
{
    ...
    cout << "*** Drived Class Object (Lerbra -> L1) ****" << endl;
    // ###### All of these Dog class public functions are protected now
    ...
    cout << "(L1) Protected - Animal `length` access by `Dog` Public Fn:- " << L1.getDogProtectedWithLerbra() << endl;
    cout << "(L1) Private - Animal `age` access by `Dog` Public Fn:- " << L1.getAnimal_DogProtectedWithLerbra() << endl;
    cout << "(L1) Public - Animal `height` access by `Dog` Public Fn:- " << L1.getDogPublicWithLerbra() << endl; // its Protected Now
    LinePrint();
